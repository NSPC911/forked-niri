...
impl State {
    ...
    pub(super) fn on_touch_motion<I: InputBackend>(&mut self, evt: I::TouchMotionEvent) {
        ...
        if let Some(old_pos) = tracked_slot {
            ...
            self.niri.touch_frame_timestamp = Duration::from_micros(evt.time().micros());
...
fn detect_edge(
    ...
    for (edge, dist) in [
        ...
        if dist < threshold && closest.is_none_or(|(_, d)| dist < d) {
...
fn feed_continuous_gesture(state: &mut State, update: ContinuousGestureUpdate) {
    let ContinuousGestureUpdate {
        kind,
        delta_x,
        delta_y,
        sensitivity,
...
/// - `frame_delta` is the signed average angular delta across fingers present in both frames, with
///   ±π unwrap applied. Positive = CCW. A noise floor of 0.001 rad is applied: smaller values clamp
///   to 0 to prevent sub-threshold drift from accumulating into a false rotation on held-still
///   fingers.
/// - `new_angles` is the fresh per-slot angle map to store for the next frame's comparison.
///
